...
from spot.io.wandb_utils import sample_and_viz_bboxes_and_nocs, sample_and_viz_bboxes_and_nocs_cherrypick
...
def train(args):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % val_every == 0 or only_validate or only_viz:
            ...
            with torch.no_grad(): # must do this to avoid running out of memory
                if CHERRY_PICK:
                    sample_and_viz_bboxes_and_nocs_cherrypick(tpointnet_encoder, viz_loader, device,max_pnts_per_batch=max_pnts_per_batch, nsamples=200, parallel=parallel, outdir=out_dir)
                else:
