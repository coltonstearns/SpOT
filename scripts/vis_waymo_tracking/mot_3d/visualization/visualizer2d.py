...
class Visualizer2D:
    ...
    def handler_box(self, box: BBox, message: str='', color='red', linestyle='solid'):
        ...
        # get orientation line
        bottom_center = np.array([box.x, box.y])
        top_center = (corners[2, :] + corners[3, :]) / 2
        orientation = np.stack([bottom_center, top_center], axis=0)
        plt.plot(orientation[:, 0], orientation[:, 1], color=self.COLOR_MAP[color], linestyle=linestyle)
