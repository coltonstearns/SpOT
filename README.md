...
This is the official implementation for the ECCV 2022 oral presentation [paper](https://arxiv.org/pdf/2207.05856.pdf).
...
```
python spot/test.py --config=configs/nusc_reported_car.yaml --out=./out/nusc_car
```

To reproduce nuScenes results on the `pedestrian` class run:
```
python spot/test.py --config=configs/nusc_reported_pedestrian.yaml --out=../out/nusc_ped
```

#### Waymo Dataset
To reproduce nuScenes results on the `vehicles` class run:
```
python spot/test.py --config=configs/waymo_reported_vehicle.yaml --out=../out/waymo_vehicle
```

To reproduce nuScenes results on the `vehicles` class run:
```
python spot/test.py --config=configs/waymo_reported_pedestrian.yaml --out=../out/waymo_ped
```
