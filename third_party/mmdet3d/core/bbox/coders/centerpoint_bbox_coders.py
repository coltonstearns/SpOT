...
class CenterPointBBoxCoder(BaseBBoxCoder):
    ...
    Args:
        ...
        Todo: increase score_threshold once we have add a classification branch!
    """
    ...
    def _topk(self, scores, K=80):
        """Get indexes based on scores. This is simply finding the top K scores across ALL classes at once!
        ...
        topk_scores, topk_inds = torch.topk(scores.view(batch, cat, -1), K)  # Gives top K for EACH CLASS (is along last dimension)

        topk_inds = topk_inds % (height * width)  # gives us a global account of the indices
        topk_ys = (topk_inds.float() /
                   torch.tensor(width, dtype=torch.float)).int().float()
        topk_xs = (topk_inds % width).int().float()

        topk_score, topk_ind = torch.topk(topk_scores.view(batch, -1), K)  # Gives us top K total across all classes, but where each class is already filtered to K total values
        topk_clses = (topk_ind / torch.tensor(K, dtype=torch.float)).int()  # Top K heatmap K-vals
        topk_inds = self._gather_feat(topk_inds.view(batch, -1, 1),  # filter from global one --> simpler!
